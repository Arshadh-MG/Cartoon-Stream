...
export function findEpisodeContext(cartoons, episodeId) {
  ...
  return {
    episode: normalizeEpisode(item, item.cartoon, item.category),
    cartoon: item.cartoon,
    category: item.category,
    previousEpisode: playlist[index - 1] ? {
      ...playlist[index - 1],
      episode: normalizeEpisode(playlist[index - 1], playlist[index - 1].cartoon, playlist[index - 1].category),
    } : null,
    ...
    playlist: playlist.map((p) => ({
      ...p,
      episode: normalizeEpisode(p, p.cartoon, p.category),
    })),
  };
}
