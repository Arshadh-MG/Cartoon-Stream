...
// Helper function to extract iframe src from HTML or URL
function extractIframeSrc(iframeUrlOrHtml) {
  const str = String(iframeUrlOrHtml || '').trim();
  
  // If it's already a URL (doesn't contain HTML), return as-is
  if (!str.includes('<iframe')) return str;
...
export default function AdvancedVideoPlayer({
  ...
  const videoType = episode?.video_type || (videoSrc.includes('<iframe') || videoSrc.includes('blogger.com') ? 'iframe' : 'file');
  const isIframe = videoType === 'iframe';
  const iframeSrc = isIframe ? extractIframeSrc(videoSrc) : '';
  ...
  async function togglePlay(event) {
    ...
    // For iframes, we can't control playback programmatically due to cross-origin restrictions
    if (isIframe) {
      return;
    }
  ...
  function skip(seconds) {
    if (isIframe) return; // Skip not available for iframes
    const video = videoRef.current;
    if (!video) return;
  ...
  function handleSeek(event) {
    if (isIframe) return; // Seeking not available for iframes
    const video = videoRef.current;
    if (!video) return;
    ...
    >
      ...
      {isIframe && iframeSrc && (
        <div className="iframe-player-container">
          <iframe
            className="advanced-video iframe-video"
            src={iframeSrc}
            frameBorder="0"
            allow="autoplay; fullscreen; picture-in-picture"
            title={episode?.name || 'Video player'}
          ...
          <div className="player-controls-row">
            ...
            <div className="player-controls-group player-controls-right">
              ...
              <input
                ...
                aria-label="Volume"
              />
              {/* Captions and quality/audio controls hidden until fully implemented */}
      ...
      {isIframe && (
        <div className="iframe-native-controls-info">
          <span>Use the player controls to play/pause and adjust settings</span>
        </div>
      )}

      {!isIframe && settingsOpen && (
        <div className="settings-popover">
          ...
          {/* Quality switching not yet implemented - video URLs don't have multiple quality options */}
          {/* Audio track switching not yet implemented - videos don't have multiple audio tracks */}
